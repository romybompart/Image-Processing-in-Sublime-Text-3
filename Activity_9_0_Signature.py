"""
	@author: romy bompart
	@title: Nineth Activity - Signature		
"""
...
segm = segmentation(filtered, 3) 

x,y = centroide2(filtered)
distance = signature(segm, x, y)

radius = np.average(distance)

print ( "Centroid - > X = {} , Y= {}".format(x,y) )
print ( "radius -> r={}".format(radius) )


plt.plot(distance, 'r')
y_c = np.ones(np.shape(distance)[0])*radius
y_line = np.arange(0, np.shape(distance)[0], 1)
